...
1. Set a variable for the domain that will be used in the rest of this deployment.
   ...
   ```bash
   export DOMAIN_NAME_AKS_BASELINE="proaircloud.eu"
   ```
...
1. Generate a client-facing self-signed TLS certificate
   ...
   ```bash
   openssl req -x509 -nodes -days 365 -newkey rsa:2048 -out appgw.crt -keyout appgw.key -subj "/CN=cloud.${DOMAIN_NAME_AKS_BASELINE}/O=ProAirCloud" -addext "subjectAltName = DNS:cloud.${DOMAIN_NAME_AKS_BASELINE}" -addext "keyUsage = digitalSignature" -addext "extendedKeyUsage = serverAuth"
...
1. Generate the wildcard certificate for the AKS Ingress Controller
   ...
   ```bash
   openssl req -x509 -nodes -days 365 -newkey rsa:2048 -out traefik-ingress-internal-aks-ingress-tls.crt -keyout traefik-ingress-internal-aks-ingress-tls.key -subj "/CN=*.aks-ingress.${DOMAIN_NAME_AKS_BASELINE}/O=ProAirCloud AKS Ingress"
   ```
