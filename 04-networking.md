...
1. Create the regional network hub.
   ...
   az deployment group create -g rg-enterprise-networking-hubs -f networking/hub-default.bicep -p location=centralus
   ```
...
1. Create the spoke that will be home to the AKS cluster and its adjacent resources.
   ...
   az deployment group create -g rg-enterprise-networking-spokes -f networking/spoke-BU0001A0008.bicep -p location=centralus hubVnetResourceId="${RESOURCEID_VNET_HUB}"
   ```
...
1. Update the shared, regional hub deployment to account for the requirements of the spoke.
   ...
   az deployment group create -g rg-enterprise-networking-hubs -f networking/hub-regionA.bicep -p location=centralus nodepoolSubnetResourceIds="${RESOURCEID_SUBNET_NODEPOOLS}"
   ```
